...
const restaurantSchema = new mongoose.Schema(
  {
    ...
    // A private, per-restaurant secret used to derive and verify every
    // table's QR code key (see utils/tableSecurity.js). This value is NEVER
    // sent to any customer's browser — only the individually-derived
    // per-table keys are, and only to the logged-in owner's dashboard when
    // generating QR codes.
    qrSecret: {
      type: String,
      default: () => require('crypto').randomBytes(24).toString('hex'),
      select: false, // Excluded from queries by default, same reasoning as Owner.passwordHash.
    },
  },
  {
